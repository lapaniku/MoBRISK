...
void MoFREAKUtilities::computeMoFREAKFromFile(std::string video_filename, std::string mofreak_filename, bool clear_features_after_computation)
{
	...
	while (true)
	{
		...
		// BRISK new version
		cv::Ptr<cv::BRISK> diff_detector = cv::BRISK::create();
		diff_detector->detect(current_frame, keypoints);
		...
		diff_detector->compute(diff_img, keypoints, descriptors);		
...
void MoFREAKUtilities::setCurrentAction(string folder_name)
{
	if (dataset == HMDB51)
	{
		...
		{
			current_action = BRUSH_HAIR;
			std::cout << "****Didn't find action" << endl;
			if(system("PAUSE")){};
			exit(1);
		}
	}
