...
void BagOfWordsRepresentation::loadClusters()
{
	...
	std::ifstream cluster_file(cluster_path);
...
void BagOfWordsRepresentation::computeHMDB51BagOfWords(string SVM_PATH, string MOFREAK_PATH, string METADATA_PATH)
{
	...
	*/
	...
	std::ofstream train1(SVM_PATH + "/split1.train");
	std::ofstream train2(SVM_PATH + "/split2.train");
	std::ofstream train3(SVM_PATH + "/split3.train");
	std::ofstream test1(SVM_PATH + "/split1.test");
	std::ofstream test2(SVM_PATH + "/split2.test");
	std::ofstream test3(SVM_PATH + "/split3.test");
	...
	for (directory_iterator dir_iter(METADATA_PATH); dir_iter != end_iter; ++dir_iter)
	{
		if (is_regular_file(dir_iter->status()))
		{
			...
			*/
			...
			std::ifstream metadata_file(current_file.string());
			string line;
...
void BagOfWordsRepresentation::intializeBOWMemory(string SVM_PATH)
{
	...
	for (int group = 0; group < NUMBER_OF_GROUPS; ++group)
	{
		...
		std::ofstream *training_filestream = new std::ofstream(training_filepath.str());
		std::ofstream *testing_filestream = new std::ofstream(testing_filepath.str());
...
void BagOfWordsRepresentation::computeBagOfWords(string SVM_PATH, string MOFREAK_PATH, string METADATA_PATH)
{
	...
	if (dataset == KTH || dataset == UCF101)
	{
		for (int i = 0; i < NUMBER_OF_GROUPS; ++i)
		{
			...
			std::ofstream *training_file = new std::ofstream(training_string.str());
			std::ofstream *testing_file = new std::ofstream(testing_string.str());
